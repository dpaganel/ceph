...
namespace rgw::sal {
  ...
  {
    dout(20) << "TRACER: making ReadOp" << dendl;
    return std::make_unique<TObject::TReadOp>(this, ctx); // Implement - Daniel P
  }

  TObject::TReadOp::TReadOp(TObject *_source, RGWObjectCtx *_rctx) : //Implement - Daniel P
    source(_source),
      ...
      optional_yield y, bool check_size_only)
  {
    ...
    return real_bucket->check_quota(dpp, user_quota, bucket_quota, obj_size, y, check_size_only);
  }
    ...
    optional_yield y)
  {
    ...
    return real_store->get_notification(dpp, obj, src_obj, rctx, event_type, _bucket, _user_id, _user_tenant, _req_id, y);
  }
  ...
  void TracerDriver::get_quota(RGWQuotaInfo& bucket_quota, RGWQuotaInfo& user_quota)
  {
    ...
    return real_store->get_quota(bucket_quota, user_quota);
  }
				  ...
				  uint64_t *cur_accounted_size) 
  {
    ...
    return real_store->get_append_writer(dpp, y, std::move(_head_obj), owner, obj_ctx, ptail_placement_rule, unique_tag, position, cur_accounted_size);
  }
				  ...
				  const std::string& unique_tag) 
  {
    ...
    return real_store->get_atomic_writer(dpp, y, std::move(_head_obj), owner, obj_ctx, ptail_placement_rule, olh_epoch, unique_tag);
  }
